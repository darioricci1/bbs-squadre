...
import { ceArchivio, segnaEvento, uno } from "./db.js";

export { tutti, uno, scrivi, aggiorna, scriviMolti, togli, ultimi, eventi, conta, contatore, correggi, ceArchivio } from "./db.js";
...
export async function creditiDi(email, utente, gia) {
  // `gia`: i contatori letti tutti insieme (statistiche), per non fare una query a persona
  const usati = gia ? gia[chiaveCrediti(email)] || 0 : await contatore(chiaveCrediti(email));
...
// IL POSSIBILE GRUPPO di un'idea: chi l'ha pubblicata, le persone proposte
// quando e' stata condivisa (proposti) e quelle aggiunte dopo (membri). E'
// un'ipotesi: le squadre vere si formano piu' avanti.
export const gruppoDi = (i) => [...new Set([i.autore, ...(i.proposti || []), ...(i.membri || [])].filter(Boolean))];
export const MAX_GRUPPO = 8;
...
export async function esigiAccesso(req, res) {
  ...
  // Sessione nata da un link d'accesso: dura a lungo, quindi si ricontrolla
  // che il link valga ancora (crearne uno nuovo la chiude).
  if (c.k) {
    const p = await uno("profili", String(c.p || ""));
    if (!p || p.codiceLink !== c.k) { res.status(401).json({ error: "Il tuo link d'accesso non vale piu'. Chiedi a Dario quello nuovo." }); return null; }
  }
...
export function profiloPubblico(p, aziende, perAdmin = false) {
  ...
  if (!perAdmin) { delete out.email; delete out.emailAttesa; delete out.linkAccesso; delete out.linkCreato; }
  return out;
}
