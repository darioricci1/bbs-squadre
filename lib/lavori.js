...
// Per ogni lavoro ci sono due descrizioni: cosa fa l'azienda (condivisa fra
// tutti quelli che ci hanno lavorato, tabella siti) e cosa faceva la persona
// (dal testo sotto il ruolo nel PDF, modificabile: profilo.lavoriMiei).
...
export function lavoriDi(p, siti) {
  ...
  return lavoriDa(p.esperienze).map((l) => {
    ...
    const mio = miei[id];
    return { ...l, id, chiave, descrizioneRuolo: mio && typeof mio.descrizioneRuolo === "string" ? mio.descrizioneRuolo : l.descrizioneRuolo, info: infoAzienda(chiave, l.azienda, siti) };
