...
//   siti         id    -> correzioni a mano su sito e settore delle aziende dove
//                         hanno lavorato le persone (id = nome normalizzato)
//   impostazioni id    -> scelte dell'amministratore (modello, effort)
...
export const TABELLE = ["profili", "aziende", "bacheca", "utenti", "generazioni", "siti", "impostazioni"];
...
/**
 * Leggi-modifica-scrivi senza perdere le modifiche degli altri: si scrive solo
 * se la riga non e' cambiata dalla lettura (xmin, la versione della riga in
 * Postgres); se e' cambiata si rilegge e si riapplica `cambia`. Se `cambia`
 * restituisce null non si scrive niente e torna null.
 */
export async function aggiorna(t, id, cambia, tentativi = 6) {
  const s = await db();
  for (let k = 0; k < tentativi; k++) {
    const [r] = await s.query(`SELECT dati, xmin::text AS v FROM ${tabella(t)} WHERE id = $1`, [String(id)]);
    if (!r) return null;
    const nuovo = await cambia(r.dati);
    if (nuovo == null) return null;
    const ok = await s.query(`UPDATE ${tabella(t)} SET dati = $2::jsonb, aggiornato = now() WHERE id = $1 AND xmin::text = $3 RETURNING id`,
      [String(id), JSON.stringify(nuovo), r.v]);
    if (ok.length) return nuovo;
  }
  throw new Error("Troppe modifiche nello stesso momento: riprova.");
}
...
/** Le ultime generazioni di una persona (non le ultime di tutti, filtrate). */
export async function generazioniDi(email, quanti = 30) {
  const s = await db();
  const righe = await s.query(`SELECT dati FROM generazioni WHERE dati->>'chi' = $1 ORDER BY creato DESC LIMIT $2`, [String(email), quanti]);
  return righe.map((r) => r.dati);
}
...
/** Leggi un contatore senza toccarlo (0 se non c'e' o e' scaduto). */
export async function contatore(chiave) {
  const s = await db();
  const [r] = await s.query(`SELECT n FROM contatori WHERE chiave = $1 AND scade > now()`, [chiave]);
  return r ? Number(r.n) : 0;
}

/** Tutti i contatori validi che iniziano con `prefisso`, in una query: { chiave: n }. */
export async function contatoriCon(prefisso) {
  const s = await db();
  const righe = await s.query(`SELECT chiave, n FROM contatori WHERE chiave LIKE $1 AND scade > now()`, [prefisso.replace(/[%_\\]/g, "\\$&") + "%"]);
  return Object.fromEntries(righe.map((r) => [r.chiave, Number(r.n)]));
}

/** Aggiunge `delta` a un contatore esistente (per restituire un credito). */
export async function correggi(chiave, delta) {
  const s = await db();
  await s.query(`UPDATE contatori SET n = GREATEST(0, n + $2) WHERE chiave = $1`, [chiave, delta]);
}
